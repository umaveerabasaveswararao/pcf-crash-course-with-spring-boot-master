...
public class CurrencyConversionController {
	...
	@Value("${CONVERSION_PROFIT_PERCENTAGE}")
	private int converionProfitPercentage;
	...
	public CurrencyConversionBean convertCurrency(@PathVariable String from, @PathVariable String to,
		...
		return new CurrencyConversionBean(response.getId(), from, to, response.getConversionMultiple(), quantity,
				convertedValue, response.getExchangeEnvironmentInfo(), conversionEnvironmentInfo,converionProfitPercentage);
	}
	...
	public CurrencyConversionBean convertCurrencyFallback( String from, String to,
		...
		return new CurrencyConversionBean(0L, from, to, BigDecimal.ZERO, quantity,
				BigDecimal.ZERO, "Error occured", "Error occured",0);
	}
}
